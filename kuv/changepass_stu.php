...
if(isset($_POST['btn-save'])){
	...
	$email=$_SESSION['Suser'];
	$passwd=$_POST['opasswd'];
	$npasswd=$_POST['npasswd'];
	$sel = "SELECT `Stud_id` FROM `student` WHERE `Stud_id`='$email' and `Password`='$passwd'";
	
	$result=mysqli_query($con,$sel);
	
	if(mysqli_affected_rows($con)==1){
		
		$ins = "UPDATE `student` SET `Password`='$npasswd' WHERE `Stud_id`='$email' and `Password`='passwd'";
