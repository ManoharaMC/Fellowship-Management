...
if(isset($_POST['btn-save'])){
	...
	$email=$_SESSION['fuser'];
	$passwd=$_POST['opasswd'];
	$npasswd=$_POST['npasswd'];
	$sel = "SELECT `Emp_id`, `Emp_name`, `emp_email`, `Dept_id`, `Desg_id`, `Specialization`, `Password` FROM `faculty` WHERE Emp_id='$email' and Password='$passwd'";
	
	$result=mysqli_query($con,$sel);
	
	if(mysqli_affected_rows($con)==1){
		
		$ins = "UPDATE `faculty` SET `Password`='$npasswd' WHERE `Emp_id`='$email' and `Password`='$passwd'";
